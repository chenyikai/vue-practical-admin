...
import { detailPoiTInfoShore } from "@/api/data-manage/index.js";
...
function open(status, data = {}) {
  dialog.value.open();
  nextTick().then(() => {
    detailFunc.value = detailPoiTInfoShore;
...
function jsonDataToObject() {
  ...
  getIconData(() => {
    if (
      formStatus.value === website.pageStatus.UPDATE &&
      !validatenull(formData.value.geom)
    ) {
  ...
  if (
    formStatus.value === website.pageStatus.UPDATE &&
    !validatenull(formData.value.extension)
  ) {
