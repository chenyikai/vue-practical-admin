...
import {
  ...
  batchDeletePoiTInfo,
  addFeedbackPoiTInfo,
  updateFeedbackPoiTInfo,
  auditFeedbackPoiTInfo,
  publishPoiTInfo,
  syncDataOnePoiTInfo,
  auditSyncDataPoiTInfo,
  downloadTemplatePoiTInfo,
  exportDataPoiTInfo,
  restPublishPoiTInfo,
} from "@/api/data-manage/index.js";
import { getPoiTTypeAllType } from "@/api/type-manage/index.js";
...
function getDicData(callback) {
  Promise.all([getPoiTTypeAllType()]).then(([lxData]) => {
...
function onUpdateFeedbackSubmit(formData, done) {
  // 应用级修改反馈
  updateFeedbackPoiTInfo({ ...formData, feedbackStatus: 1 })
    .then(() => {
      done(true);
      ElMessage({
        message: "修改反馈成功",
...
function onAuditFeedbackSubmit(formData, done) {
  // 超管审核
  auditFeedbackPoiTInfo(formData)
    .then(() => {
      done(true);
      ElMessage({
        message: "审核成功",
...
function deletesDataSubmit() {
  if (toggleList.value.length > 0) {
    ...
    ElMessageBox.confirm(`是否确认删除${ids.length}条数据？`, "提示", {
      ...
      .then(() => {
        return batchDeletePoiTInfo(ids);
      })
      .then(() => {
        ElMessage({
          message: "删除成功!",
...
function syncDataSubmit() {
  if (toggleList.value.length > 0) {
    ...
    ElMessageBox.confirm(`是否确认发布${ids.length}条数据？`, "批量发布提示", {
      ...
      .then(() => {
        return restPublishPoiTInfo(ids);
      })
      .then(() => {
        ElMessage({
          message: "批量发布成功!",
...
function publishGmDataSubmit(rowData) {
  ElMessageBox.confirm(`是否确认发布：${rowData.name}？`, "发布", {
    ...
    .then(() => {
      return publishPoiTInfo([rowData.id]);
    })
    .then(() => {
      ElMessage({
        message: "删除成功!",
...
function onRevPublishSubmit(formData, done) {
  // 审核同步内容
  auditSyncDataPoiTInfo(formData)
    .then(() => {
      done(true);
      ElMessage({
        message: "审核成功",
...
function onDownloadTemplate() {
  ...
  if (listQuery.typeCode) {
    downloadTemplatePoiTInfo({ typeCode: listQuery.typeCode }).then(
      ({ data }) => {
        downloadBlobStream(data, "海上数据导入模板.xlsx", () => {
          ElMessage({
            message: "导出模板成功",
            type: "success",
          });
...
function onExportData() {
  ...
  if (listQuery.typeCode) {
    ...
    exportDataPoiTInfo({ typeCode: listQuery.typeCode }).then(({ data }) => {
