...
import {
  createPoiTAttribute,
  deletePoiTAttributeById,
  getPoiTAttributePage,
  updatePoiTAttribute,
} from "@/api/attribute-manage/index.js";
...
function onDelete(rowData) {
  ElMessageBox.confirm(
    `是否确认删除属性：${rowData.attributeName}？<div style="color:#ff4757">此操作将会导致所有相关数据一并删除！</div>`,
    "警告",
    {
    ...
    .then(() => {
      return deletePoiTAttributeById(rowData.id);
    })
    .then(() => {
      ElMessage({
        message: "删除成功!",
...
onBeforeMount(() => {
  funcList.page = getPoiTAttributePage;
  getList();
});
</script>
